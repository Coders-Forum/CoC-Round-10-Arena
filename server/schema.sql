-- ═══════════════════════════════════════════════════════════════════
-- CLASH OF CODERS — COMPLETE SUPABASE DATABASE SCHEMA
...
CREATE TABLE IF NOT EXISTS public.teams (
    ...
    phase1_lands JSONB DEFAULT '[]'::jsonb,
    phase2_lands JSONB DEFAULT '[]'::jsonb,
    phase3_lands JSONB DEFAULT '[]'::jsonb,
...
-- 2. Create contest_state table for all global contest settings
CREATE TABLE IF NOT EXISTS public.contest_state (
    ...
    bypass_login BOOLEAN DEFAULT FALSE,
    active_results_phase TEXT DEFAULT 'phase1',
    eliminated_teams JSONB DEFAULT '[]'::jsonb,
    manual_ranks JSONB DEFAULT '{}'::jsonb,
...
-- 3. Create admin_sessions table to persist admin logins across serverless cold starts & hard refreshes
...
-- 4. Create team_sessions table to persist user logins across serverless cold starts & hard refreshes
...
-- 6. Add Permissive RLS Policies (Allows full access for both service_role & anon_key)
...
DROP POLICY IF EXISTS "Public access on admin_sessions" ON public.admin_sessions;
CREATE POLICY "Public access on admin_sessions" ON public.admin_sessions FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Public access on team_sessions" ON public.team_sessions;
CREATE POLICY "Public access on team_sessions" ON public.team_sessions FOR ALL USING (true) WITH CHECK (true);


-- 7. Seed initial row in contest_state
INSERT INTO public.contest_state (id, active_stage, disabled_lands, bypass_login, active_results_phase, eliminated_teams, manual_ranks)
VALUES ('current', 'round1', '[]'::jsonb, false, 'phase1', '[]'::jsonb, '{}'::jsonb)
